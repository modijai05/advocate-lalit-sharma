...
export const RepresentativeMatters: React.FC = () => {
  return (
    <section className="relative bg-[#F4F4F2] text-[#111111] py-16 sm:py-24 border-b border-[#D8D8D5]">
      <div className="absolute inset-0 pointer-events-none opacity-[0.07] bg-[radial-gradient(#000_0.5px,transparent_0.5px)] [background-size:24px_24px]" />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-10 relative z-10">
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <div className="mt-4 inline-flex items-center gap-1.5 text-[0.7rem] font-ui text-[#33312C] bg-black/[0.04] px-3 py-1.5 border border-black/10 rounded-sm">
            <ShieldCheck className="w-3.5 h-3.5 text-[#111] shrink-0" />
        ...
        <div className="divide-y divide-black/10 border-y border-black/10">
          {REPRESENTATIVE_MATTERS.map((matter, index) => (
            <motion.div
              ...
              className="py-5 grid grid-cols-1 sm:grid-cols-[1fr_2fr] gap-4 items-start hover:bg-black/[0.03] px-3 transition-colors rounded-sm"
            >
              <div className="flex items-start gap-3">
                <span className="text-[0.65rem] font-bold text-[#111] bg-black/8 px-2 py-0.5 rounded-sm mt-1 shrink-0 border border-black/12">
                  {matter.code}
                </span>
                <div>
